...
const { createOrganization, getStatusId } = require('./setup.js');
...
Then('a organização {int} é {string} com direito de voto', async function (id, name) {
    let org = await this.organizationContract.getOrganization(id);
    assert.equal(org.name, name);
    assert.equal(org.canVote, true);
});

Then('a organização {int} é {string} sem direito de voto', async function (id, name) {
    let org = await this.organizationContract.getOrganization(id);
    assert.equal(org.name, name);
    assert.equal(org.canVote, false);
});

When('a conta {string} adiciona a organização {string} com direito de voto', async function (account, name) {
    const signer = await hre.ethers.getSigner(account);
    assert.ok(signer != null);
    await this.organizationContract.connect(signer).addOrganization(name, true);
});
